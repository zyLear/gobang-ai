 ...
 * @date 2021/6/23
 */
public class MinMaxNextPointHunterBase implements NextPointHunter {


    private int[][] tryList = {{1, 2}, {}};

    @Override
    public NextPointHuntBean getNextPointList(int[][] tryChess, int c) {
                    ...
                    for (int i = -2; i <= 2; i++) {
                        for (int j = -2; j <= 2; j++) {
                            ...
                            if (isInside(xIndex, yIndex)) {
                                if (tryChess[xIndex][yIndex] != 0) {
                                    int score = getTryPointScore(tryChess, x, y, c);
                                    if (max < score) {
                                        max = score;
                                    }
